 ...
 * loadPromptConfig reads `.pi/references/prompt-injector.yml` once per project root;
...
/**
 * Parsed prompt configuration from prompt-injector.yml.
 * Keys are stage names (clarify, plan, develop, review, fix) plus "verify_extract".
...
/**
 * Loads the prompt configuration from `.pi/references/prompt-injector.yml`.
...
/**
 * Returns the verify_extract prompt from the yml config.
 * Falls back to DEFAULT_VERIFY_EXTRACT_PROMPT when the value is missing or empty.
 *
 * @param projectRoot - Absolute path to the project root
 * @returns The extraction prompt string (custom or default)
 */
export async function getVerifyExtractPrompt(projectRoot: string): Promise<string> {
  const config = await loadPromptConfig(projectRoot);
  const value = config["verify_extract"];
  if (value === undefined || value.trim() === "") {
